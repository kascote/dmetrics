...
Knobs that differ between the file and the run are exit 2 with a diagnostic naming the file: values counted under different rules are not comparable, and the fix is `dmetrics baseline`. A file that cannot be read or parsed is the same. A metric the file does not list is compared as if there were no baseline for it. Entries for files outside the targets are ignored, so a single-file run neither reports the rest of the baseline as gone nor counts it. An entry under the targets whose file no longer exists stays in: it takes the fingerprint pass, which follows a file move, and otherwise counts as gone.

**Matcher**, deterministic and explainable, three passes: named scopes by exact id, which survives every body edit; then, among the scopes still unmatched on both sides, by fingerprint, when the fingerprint is unique on both sides, which follows a move between files and a rename; then closures by exact id, which is the ordinal under their parent. Closures go through the fingerprint pass before the ordinal pass because inserting a closure shifts the ordinals of those after it; an edited closure whose ordinal also moved reads as new. A rename plus a body edit reads as new: you touched it, you own it. No fuzzy similarity until a field trial shows the two keys missing.
