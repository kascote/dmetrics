...
**Status:** Draft 0.12 · **Owner:** Nelson · **Date:** 2026-09-12
**Name:** `dmetrics`

**Changes in 0.13 (baseline matcher, fourth pass):** the field trial of the baseline over 13 packages across revisions found the matcher missing one class of scope: a file moved (dart_style's `lib/src/*.dart` to `lib/src/short/`) with a body edit in the same change, so neither the id nor the fingerprint is in the run; 14 fail-level and 37 warn-level results read as `new` and their entries as `gone`. The matcher gains a fourth pass (§7.4): against the entries of absent files only, by the path-stripped id when it is unique on both sides. Still no similarity heuristic; the local id is exact and carries the kind, class, name and closure ordinal. The `library` scope sits it out. Same trial, same conclusion on closures: the five closure `new` results with no match were genuinely new closures, fuzzy closure matching not warranted.

**Changes in 0.12 (M8, the baseline file):** `dmetrics baseline` records a run per config root and `analyze` compares against it (§7.1, §7.4, `baseline:` in §8). Motivation: thresholds catch the tail, deltas catch the drift; at month five a strict CI needs to fail on new and worse code without a cleanup of the legacy first, and a scope creeping toward a threshold should be visible before it crosses. The file holds every scope's rolled-up values and fingerprint, no verdicts and no thresholds, so tightening a threshold grows the baselined count instead of failing the run; one scope per line, paths relative to the file, ids stored without their path through two engine helpers (`ScopeId.localIn`, `ScopeId.inFile`) so no reporter parses an id. The matcher is three passes, exact id, unique fingerprint, closure ordinal, with no similarity heuristic; the fingerprint now hashes the declaration's own name tokens as a placeholder so a pure rename matches, the one change the baseline needed from it, and it is frozen from here. Statuses `new`, `worse`, `baselined`, `changed`, `unchanged` per result with the rule that exit 1 means new or worse, `fixed` and `gone` counted for the refresh nudge; the console shows the status in the tag, dims baselined debt like a suppression, and prints the drift below the floor as a section; the JSON report gains `baseline` on compared results, configs and the summary, present only with a baseline. A partial run refreshes only the files under its targets; different counting knobs are exit 2. Built like `stats`: `src/baseline` is pure (format, matcher, comparison), `src/io/baseline_files.dart` reads and writes, and `RunResult.hasViolations` consults the comparison. The agent guide gained a section, the one rule it stresses being never to refresh the baseline to make a run pass. `renderConsole` crossed cognitive 25 while gaining the section and was split by responsibility. Radar updated in §8.
...
dmetrics analyze [<file>|<dir> ...] [--json] [--json-contributors full|summary] [--all]
              ...
              [--baseline <path>] [--no-baseline] [--top N]
```
...
- `--baseline <path>` compares against one baseline file for every root (§7.4), the way `--config` forces one root; `--no-baseline` compares against nothing, so every violation counts. Without either, each root's configured baseline applies when the file exists. `--top` caps the changed-scopes section of the console output (default 10).
...
```
dmetrics baseline [<file>|<dir> ...] [--config <path>] [--set <key>=<value>] [--output <path>]
```

- Measures exactly as `analyze` does, then records the run in each config root's baseline file (§7.4): the scopes of every file in the run, values only. Entries for files under the run's targets replace what the file held for them; entries for files outside the targets are kept, so `dmetrics baseline lib/src/parser.dart` refreshes one file without discarding the rest. A file recorded under different run-global knobs is rewritten whole, since its values are not comparable. Refuses to write when analysis is incomplete (exit 2): a baseline over recovered ASTs would accept numbers that are not the code's. `--output` names the file for a single-root run and is a usage error with several roots. Prints one line per file written with the scope count and how many results sit at or above the current thresholds; exit 0.

 (`schemaVersion`, `tool`, `status`, `summary`, `diagnostics`, `libraries[]` with `fanOut`, `fanIn`, `exports`, `instability`, `cycle`, `verdict`; `cycles[]` with `members` and `backEdges`; `directories` with `depth`, `order` and `edges`). Exit codes as `stats`.
...
- **`fingerprint`** is FNV-1a (32-bit) over the declaration's token lexemes, NUL-separated, comments and whitespace excluded, as eight hex digits. The declaration's own name tokens (a method's name, a constructor's class and name) hash as a fixed placeholder, so renaming a scope without touching its body keeps its fingerprint and the baseline matcher (§7.4) can follow the rename; other identifiers are not normalized, so a body edit changes it.

### 7.4 Baseline

Thresholds catch the tail; deltas catch the drift. The baseline is a snapshot of a run that `analyze` compares against, so CI can be strict on new code in a codebase that already fails, and so a function creeping from 4 to 9 under a threshold of 10 is visible before it crosses. It is a diff against one snapshot, not a history (N5).

**File.** One per config root, JSON, at the path the root's `baseline:` key names, relative to the root; default `dmetrics_baseline.json` next to the root's `analysis_options.yaml` (or at the package root for a defaults root), used when the file exists, `baseline: none` to opt out. A root's file records the run-global knobs (`run`, without `fail_on`, which does not change a value), the metric ids covered, and one entry per scope keyed by file path relative to the file's own directory and by the scope id with its path stripped (`method:Parser.parseExpr`, `method:Parser.parseExpr::closure#1`, `library`), holding the fingerprint and the rolled-up `value` per metric. No verdicts and no thresholds: the comparison applies the current thresholds to both sides, so tightening a threshold does not fail CI on legacy code, it grows the baselined count. One scope per line, sorted by path then id, so the file's git diff is the debt ledger's history: adding debt means committing added lines with high values, paying it down means the file shrinks. On load, paths are rebased to the run root and ids rebuilt; the engine owns the id shape (`ScopeId.localIn`, `ScopeId.inFile`) and the baseline never parses one.

Knobs that differ between the file and the run are exit 2 with a diagnostic naming the file: values counted under different rules are not comparable, and the fix is `dmetrics baseline`. A file that cannot be read or parsed is the same. A metric the file does not list is compared as if there were no baseline for it. Entries for files outside the targets are ignored, so a single-file run neither reports the rest of the baseline as gone nor counts it. An entry under the targets whose file no longer exists stays in: it takes the fingerprint pass, which follows a file move, then the local-id pass, which follows a move plus an edit, and otherwise counts as gone.

**Matcher**, deterministic and explainable, four passes: named scopes by exact id, which survives every body edit; then, among the scopes still unmatched on both sides, by fingerprint, when the fingerprint is unique on both sides, which follows a move between files and a rename; then closures by exact id, which is the ordinal under their parent; then, against the entries of files no longer in the tree only, by the id with its path stripped (`method:Parser.parseExpr`, closure ordinal included), when it names exactly one unmatched scope in the run and exactly one entry of an absent file, which follows a scope whose file moved and whose body was edited in the same change. The `library` scope sits out the last pass: its local id carries no name and every file has one. Closures go through the fingerprint pass before the ordinal pass because inserting a closure shifts the ordinals of those after it; an edited closure whose ordinal also moved reads as new. A rename plus a body edit reads as new: you touched it, you own it. No fuzzy similarity: the baseline field trial (13 packages, 2026-09-12) showed the moved-and-edited scope as the one miss class the first three passes left, 14 fail-level results across the corpus, and the local id closes it.

**Status**, one per result, with `floor` the `fail_on` verdict:

| Status       | Match | Verdict | Value          | Counts as a violation |
| ------------ | ----- | ------- | -------------- | --------------------- |
| `new`        | none  | any     | —              | when verdict ≥ floor  |
| `worse`      | yes   | ≥ floor | above baseline | yes                   |
| `baselined`  | yes   | ≥ floor | at or below    | no                    |
| `changed`    | yes   | < floor | differs        | no                    |
| `unchanged`  | yes   | < floor | equal          | no                    |

So exit 1 means new or worse: a baselined 13 that becomes 15 fails, a baselined 13 that becomes 12 does not, and a 6 that becomes 9 under `warn: 10` is drift, reported and not failed. The run also counts `fixed` (a matched entry at or above floor by its baseline value, now below) and `gone` (an entry in a file of the run with no match). Suppressions keep their meaning: the stage order is aggregation, suppression, thresholds, baseline, and a suppressed result is never a violation whatever its status.

**Output.** Console: every finding prints as before, with the status in the tag — `fail (new)`, `fail (worse, was 11)`, `fail (baselined)` dimmed like a suppression so the accepted debt reads as background, `warn (was 6)` for a changed result that prints anyway. After the findings, a `Changed since baseline` section lists the changed results that did not print as findings (ok, not suppressed), largest delta first, `--top` rows, both directions, so the drift is on screen; `--all` prints everything in the main list instead. The summary gains a `baseline:` clause with the nonzero counts (`baselined`, `new`, `worse`, `fixed`, `changed`, `gone`) and, when `fixed` or `gone` is nonzero, a line suggesting `dmetrics baseline`. JSON: each compared result gains `baseline: { "status", "value" }` (`value` null for `new`), `configs[]` gains `baseline` (the file's run-root-relative path or `null`), and `summary` gains `baseline` with the six counts; all three present only when a baseline was in play, like `detail`. `status` keeps its meaning: `violations` is now new or worse. `stats` and `deps` never load a baseline; they have no verdict.

**CI.** One job: `dmetrics analyze --json` as the artifact, the exit code as the gate, the console output as the job summary. Refreshing is a deliberate `dmetrics baseline` commit, reviewed like any other; a stale file never breaks matching for named scopes, since id wins.
...
- **Per-root (may differ between roots):** thresholds, metric enablement, `overrides` path-glob blocks, include/exclude patterns, the baseline path.
...
9. **M8 — Baseline file.** The feature that makes the tool matter at month five: thresholds catch the tail, deltas catch the drift. `dmetrics baseline` records a run per config root, `analyze` compares against it by id then fingerprint then ordinal, exit 1 means new or worse, baselined violations still print, marked, and the changed scopes below the floor print as a section (§7.4). Built like `stats`: a pure comparison over the finished run, the file read and written by the I/O layer, the engine touched only for the fingerprint's name placeholder and the id rebasing helpers. Decided before writing code, to be revisited once it runs: every scope in the file rather than violations only, accepting the file size (dmetrics itself: 470 scopes, about 30 KB); a baselined failure that gets worse fails; renames match through the fingerprint; the baseline activates by config, `--no-baseline` opts out; no fuzzy closure matching.

**Radar (post-M8, unscheduled):** `--dot` for `deps` (Graphviz of the directory graph) if the console layering proves not enough → Martin-style abstractness per package, which needs class kinds and so the resolved pipeline (instability is in `deps` per library) → a cycle metric with a verdict, once there is data on which back edges matter (§6.5; `deps` now names them) → editor shell as reviewer tooling → the resolved pipeline when a metric needs elements (CBO per class, inheritance depth, dead code; §9).
...
- **Baseline matching — decided at M8 (§7.4).** Exact id, then unique fingerprint, then closure ordinal; no similarity heuristic until a field trial shows the two keys missing. The known tension (an edited closure changes its fingerprint, an inserted one shifts later ordinals) is accepted as "new" for the edited-and-shifted case. The fingerprint now masks the declaration's own name, the one change the baseline needed from it; it is frozen from here, since every baseline file depends on it. `id` stays opaque to reporters: the baseline strips and restores the path through engine helpers.
