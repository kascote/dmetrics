...
**Changes in 0.7 (M4 field trial):** `table-shaped: <kind>` marker on console lines and `tableShaped: { kind, share }` in JSON results when one contributor kind supplies ≥ 80% of a scope's increments (≥ 8 in total). Motivation: across three trial codebases the only false fails were pure `case` tables (`_contributorFor`, a text editor's `_executeAction`) and field-wise `==`/`copyWith`; the scalar cannot separate `case ×20` from `case ×12, if ×12`, the breakdown can. A reading hint for consumers, not a counting change (N6 intact).
...
- Console output is `dart analyze`-shaped: one line per finding (`path:line:col • verdict • kind qualifiedName • metric value [warn ≥ w, fail ≥ f] • contributor summary • table-shaped: kind` (last field only when one kind dominates), only warn / fail / suppressed results and diagnostics by default, every scope with `--all`, then a one-line summary ending in the status.
...
- **Table-shaped scopes are marked, not excused.** When one contributor kind supplies at least 80% of a scope's summed increments and the total is at least 8, the result carries `tableShaped: { kind, share }` (JSON, present only then) and the console line ends in `table-shaped: kind`. A `switch` over twenty messages, a seventeen-field `==`, a `copyWith` of `??`s: their score is the size of a table, and a consumer deciding whether to refactor should read the marker first. The verdict is unchanged; getting such a scope past CI is what a named override (§12) or a suppression is for. Metric-agnostic: computed from kinds and increments alone.
