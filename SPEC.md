...
**Status:** Draft 0.10 · **Owner:** Nelson · **Date:** 2026-09-12
**Name:** `dmetrics`
...
**Radar (post-M6, unscheduled):** baseline file for CI adoption (its matcher consumes `id` and `fingerprint`, §7.3) → Martin-style instability and abstractness per package on the graph `coupling` already reports (`dependents` and `dependencies` are `Ca` and `Ce`) → a cycle metric with a verdict, once there is data on what breaks a component (§6.5) → editor shell as reviewer tooling → the resolved pipeline when a metric needs elements (CBO per class, inheritance depth, dead code; §9).
